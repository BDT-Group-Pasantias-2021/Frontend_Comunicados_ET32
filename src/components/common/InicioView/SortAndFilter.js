...
export default function SortAndFilter({ filterComunicadosByDate, filterComunicadosByTag }) {
	...
	useEffect(() => {
		...
		if (filterMenu) {
			if (showFilterMenu) {
				filterMenu.classList.add('filter-menu-container-active');
			} else {
				filterMenu.classList.remove('filter-menu-container-active');
			}
		}
		...
		if (inputFilter) {
			/* inputFilter.addEventListener('input', (event) => {
				setFilterValue(inputFilter.value);
	...
	return (
		<div className="filter-bar-container col-11 col-sm-12 col-md-10">
			...
			<div className="standard-icon-container standard-icon-container-margin filter-btn-toggler">
				<span className="filter-btn-content" onClick={() => setShowFilterMenu(!showFilterMenu)}>
				...
				<div className="filter-menu-container" id="filterMenu">
					<div className="filter-option-btn" onClick={() => setFilterType('none')}>
						Mostrar todo
					</div>
					<div className="filter-option-btn" onClick={() => setFilterType('date')}>
						Fecha
					</div>
					<div className="filter-option-btn" onClick={() => setFilterType('number')}>
						Categoría
					</div>
					<div className="filter-option-btn" onClick={() => setFilterType('text')}>
						Emisor
					</div>
			...
			{/* <Dropdown isOpen={dropdownOpen} toggle={openDropdown}id="select-comunicados">
						...
						<DropdownMenu>
							...
							<DropdownItem value="idCategoria">
								Categoria
							</DropdownItem>
						</DropdownMenu>
					</Dropdown> */}
